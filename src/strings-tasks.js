...
function sumOfCodes(str) {
  return str ? str.split('').reduce((sum, c) => sum + c.charCodeAt(0), 0) : 0;
}
...
function formatTime(minutes, seconds) {
  const minutesStr =
    Math.trunc(minutes / 10) > 0 ? `${minutes}` : `${minutes}`.padStart(2, '0');
  const secondsStr =
    Math.trunc(seconds / 10) > 0 ? `${seconds}` : `${seconds}`.padStart(2, '0');
...
function isPalindrome(str) {
  const palindrome = str.toLocaleLowerCase().replace(/\W/g, '').split('');
  for (let i = 0; i <= Math.trunc(palindrome.length / 2); i += 1) {
...
function findLongestWord(sentence) {
  return sentence.split(' ').reduce((words, w) => {
    if (words.length < w.length) return w;
    return words;
  }, '');
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (c) => rot13[rot13.indexOf(c) + 13]);
}
...
function getCardId(value) {
  ...
  const suit = [9827, 9830, 9829, 9824];
  const s = suit.indexOf(value.slice(-1).charCodeAt(0));
  return 13 * s + cards.indexOf(value.slice(0, -1));
}
